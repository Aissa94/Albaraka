...
require_once 'db.php';
$data = new Spreadsheet_Excel_Reader("positions.xls");
  ...
  if (isset($data->sheets[$i]['cells']))
  {
    if(count($data->sheets[$i]['cells'])>0) // checking sheet not empty
    {
      ...
      for($j=2;$j<=count($data->sheets[$i]['cells']);$j++) // loop used to get each row of the sheet
      {
        ...
        $description =  mysqli_real_escape_string($connection,$data->sheets[$i]['cells'][$j][3]);
                $query = "insert into positions(id,name,description)
